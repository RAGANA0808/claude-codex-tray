...
固定色の帯ではどう調整しても一致しません。そこで float のときは**背景そのものを
透過**させ、本物のタスクバーを透かせています（`config.json` の
`float_transparent`、既定 `true`）。

透過した部分はクリックが下へ抜けるため、帯の左端に**掴むための細い縦線**を
残してあります。ここをドラッグすると移動できます。
...
| `float_transparent` | float 時に背景を透過してタスクバーを透かす。既定 `true` |
