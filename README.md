...
タスクバーなら埋め込めます）。埋め込めないと判定したときは、その理由を通知します。
...
Claude Code の認証が切れています。PowerShell で `claude auth login` を実行して
ください。ダッシュボード（帯をダブルクリック）にも同じ案内が出ます。
実行しても直らない場合は、通知領域アイコンの右クリック →「診断情報を表示」の
`★再ログインが必要と判定しています` の行に、そう判断した理由（Claude CLI の
出力）が載ります。
