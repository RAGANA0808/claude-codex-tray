...
Codex は**動いている間しか**使用量を記録しません（`~/.codex/sessions/` の
rollout ファイルに書かれます）。一方でプランの枠はアカウント単位で、デスクトップ
アプリや Codex Cloud など、このツールから見えない経路でも消費されます。

そのため Codex を止めている間は数値が更新されず、実際の使用量はモニターの表示より
**多く**なります。最後の計測から一定時間が過ぎた値にはアイコンへ `~` を付け、
ダッシュボードには計測時刻と経過時間を出します（閾値は `codex_stale_minutes`、
既定 10 分）。`~` が付いていたら Codex アプリ側の表示を正としてください。

並行セッションの扱いも変えています。ファイルの更新時刻は書き込みのたびに動くため、
「最も新しく更新されたファイル」が最も新しい計測値を持つとは限りません。現在も
続いている枠の計測値のうち**最大のもの**を採用します（枠の中で使用量は増える一方
なので、最大値が現在値に最も近い）。
...
| `codex_stale_minutes` | Codex の計測値が何分古くなったら `~` を付けるか。既定 10 |
