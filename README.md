...
- **ダブルクリック**で詳細ダッシュボード
- **右クリック**で隠す／表示設定／埋め込み切替／位置リセット／更新／終了

#### 表示する項目を選ぶ

Claude だけ、Codex だけを使っている場合は、通知領域アイコンかウィジェットの
右クリックから **「表示設定…」** を開き、不要な側のチェックを外せます。外した側は
帯から消え、**幅もその分だけ縮みます**（両方を外すことはできません）。
`config.json` の `show_claude` / `show_codex` に保存されます。

帯の幅は固定値ではなく、実際のフォントで測った列幅から組み立てています。以前は
パーセント欄の幅が足りず、`100%` になるとリセットまでの残り時間に重なって読めなく
なっていました。
...
埋め込めるかどうかは**実測で判断**します。タスクバーが子ウィンドウの描画を自分の
背景へ加算合成する機種では、明るい帯の上に背景より暗い色を置けず文字が読めないため、
その場合だけライト配色のまま `float` で帯の上に重ねて表示します（不透明なライトの
タスクバーなら埋め込めます）。埋め込めないと判定した場合は、その理由を通知します。

#### float でもタスクバーに溶け込みます

透明効果が有効なタスクバーは壁紙の色を拾うため、場所によって色が変わります。
固定色では一致しないので、float のときは**すぐ隣のタスクバーの色を実測して
背景に使います**（`config.json` の `float_match_taskbar`、既定 `true`）。

背景そのものを透過させて本物のタスクバーを透かす方式もあります
（`float_transparent`、既定 `false`）。色は完全に一致しますが、ウィンドウが
レイヤード扱いになるため環境によっては表示が不安定になります。透過中は帯の
左端に**掴むための細い縦線**が出ます（透過部分はクリックが下へ抜けるため）。

#### float が点滅する場合

Windows 11 のタスクバーは自分を最前面へ押し上げ直すため、その上に浮かべた帯は
数秒で下へ潜ってしまいます。以前は再前面化が4秒間隔だったので、帯が数秒おきに
現れては消えていました。現在は `float` のとき 250ms ごとに前面を取り直します
（埋め込み時は競合しないので低頻度のままです）。

画面がロックされていたり、他のウィンドウがタスクバーを覆っていると、測定しても
他人のピクセルを読むだけです。その場合は判定を保留し、30秒ごとに見直します。
それでも描画が画面に届かないと判定した場合は、いったん `float` に切り替えたうえで
5分ごとに埋め込みを再挑戦します（一時的な状態で表示モードが固定されないように）。
...
較正用の測定も帯全体ではなく中央の小さな区画だけを塗るので、途中で中断されても
残るのは数ピクセルです。
...
Claude Code の認証が切れています。**通知領域アイコンを右クリック →
「Claude にログイン…」**（または帯をダブルクリックしてダッシュボードの
「Claude にログイン」ボタン）を選ぶと、サインイン画面が開きます。案内に従って
完了すれば数値は自動で戻ります。

サインインは Claude Code 本体が行い、**このアプリは認証情報を一切扱いません**。

直らない場合は、右クリック →「診断情報を表示」の `auth status` と
`★再ログインが必要と判定しています` の行をご確認ください。そう判断した理由
（Claude Code の出力）も載ります。
...
**local**（フォールバック）: live が使えない場合は `~/.codex/sessions/` の rollout
...
| `show_claude` / `show_codex` | 帯に表示する側。「表示設定…」から変更可。既定はどちらも `true` |
| `taskbar_bg` | `auto`（起動時に実測・既定）/ `#rrggbb`（固定）/ `#000000`（補正なし） |
| `theme` | `auto`（Windows のタスクバー配色に追従・既定）/ `light` / `dark` |
| `float_match_taskbar` | float 時に隣のタスクバーの色を実測して背景に使う。既定 `true` |
| `float_transparent` | float 時に背景を透過する。色は完全一致だが環境により不安定。既定 `false` |
